...
function validateBigint(layout, bigint) {
    validateLayout(layout);
    var totalBits = layout.reduce((acc, curr) => acc + curr.bits, 0);
...
function decode(layout, bigint) {
    ...
    for (var i = 0; i < layout.length; i++) {
        var field = layout[i];
        const mask = BigInteger.maxValue(field.bits).shiftLeft(currentBit);
        const value = bigint.and(mask).shiftRight(currentBit);
